...
@section('content')
    <style>
        ...
        .stock_in_report_list_by_default th {
            overflow: hidden;
    ...
    <section class="home-section">
        ...
        <div class="home-content" style="margin-left: 20px">
            <div class="table_buttons_container mb-3"
                ...
                {{-- <a href="{{ route('prints#stockInDefaultPrint') }}" class="btnPrint"><button class="btn btn-primary"><i
            ...
            <div class="stock_in_report_container row">
                <div class="col-3 stock_in_filter stock-in-report-left">
                    ...
                    <div style="display: flex; justify-content:end">
                        <button class="btn custom_btn" id="btn_stock_in_search" name="btn_stock_in_search">Search</button>
                    </div>
                </div>
                <div class="col-9 stock-in-report-right">
                    <div class="stock_in_table p-3">
                                ...
                                <tbody>
                                    ...
                                    @if (count($stock_receive_list) != 0)
                                        @php
                                            $count = 1;
                                        @endphp
                                        @foreach ($stock_receive_list as $stock_receive)
                                            <tr>
                                                <td style="text-align: center">{{ $count }}</td>
                                                <td>{{ date('d-M-y', strtotime($stock_receive['receive_date'])) }}</td>
                                                <td>{{ $stock_receive['receive_voucher_number'] }}</td>
                                                <td>{{ number_format($stock_receive['total_amount']) }}</td>
                                                <td style="word-wrap: break-word; white-space:normal;">
                                                    {{ $stock_receive['remark'] }}</td>
                                            </tr>
                                            @php
                                                $count++;
                                                $totalAmount += $stock_receive['total_amount'];
                                            @endphp
                                        @endforeach
                                    @endif
                                </tbody>
                    ...
                    <div class="total_div d-flex">
                        <label class="form-label total-amount-lbl">Total Amount:</label>
    ...
    <script src="{{ asset('script/stock_in_report_script.js') }}"></script>
